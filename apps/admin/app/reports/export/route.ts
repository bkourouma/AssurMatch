...
/**
 * Authenticated proxy for the admin activity export: the session token lives in an httpOnly
...
export async function GET(): Promise<Response> {
  ...
  try {
    const response = await fetch(`${backOfficeApiBaseUrl()}/admin/dashboard/export.csv`, {
    ...
    return new Response(await response.text(), {
      ...
      headers: {
        ...
        "content-disposition": `attachment; filename="rapport-plateforme-${new Date().toISOString().slice(0, 10)}.csv"`
      }
    });
  } catch {
