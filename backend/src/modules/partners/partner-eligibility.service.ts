...
export class PartnerEligibilityService {
  ...
  evaluate(partnerTenantId: string, countryId: string, productId?: string): PartnerEligibilityResult {
    ...
    if (productId && !this.partners.isAuthorizedForProduct(partnerTenantId, productId)) reasons.push("partner_product_not_authorized");
