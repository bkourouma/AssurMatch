...
describe("BrokerStarter dashboard", () => {
  it("aggregates counters only for the connected broker tenant", () => {
    ...
    const leadA = assignments.create({ quoteRequestId: "q1", partnerTenantId: "broker-a", assignmentReason: "routing" }, actor);
    assignments.updateStatus(leadA.id, "accepted", actor, "accepted");
    assignments.create({ quoteRequestId: "q2", partnerTenantId: "broker-b", assignmentReason: "routing" }, { ...actor, partnerTenantId: "broker-b" });

    expect(leads.dashboard(actor)).toEqual({ received: 1, seen: 0, accepted: 1, rejected: 0, disputed: 0 });
  });
});
