...
describe("QuoteRoutingService", () => {
  it("selects a single eligible broker and ignores sponsorship semantics", () => {
    ...
    const partner = partners.create({ legalName: "Broker A", primaryEmail: "a@broker.example", primaryWhatsApp: "+2250102030405", status: "active" }, superAdminActor);
    ...
    licenses.create({ partnerTenantId: partner.id, licenseNumber: "LIC", issuingAuthority: "Regulator", countryId, productIds: [productId], status: "valid", effectiveDate: "2026-01-01", expirationDate: "2030-01-01" }, superAdminActor);
    ...
    const result = service.route({ id: crypto.randomUUID(), countryId, productId, consentRecordId: crypto.randomUUID(), status: "created", routingStatus: "not_started" }, superAdminActor);

    expect(result.routingStatus).toBe("assigned");
    expect(assignments.list()).toHaveLength(1);
  });

  it("blocks routing when consent is missing", () => {
    const audit = new AuditLogWriter();
    ...
    expect(result.routingStatus).toBe("blocked");
