...
describe("runtime audit HTTP refusals", () => {
  ...
  it("persists audit entries after tenant refusal", async () => {
    ...
    const other = await harness.runtime.leads.assignments.create({ quoteRequestId: "q-runtime-b", partnerTenantId: "broker-b", assignmentReason: "routing", publicReference: "AM-B" }, { ...actor, partnerTenantId: "broker-b" });
