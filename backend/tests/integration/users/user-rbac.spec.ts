...
describe("user RBAC administration", () => {
  it("audits user creation and role assignment", () => {
    const audit = new AuditLogWriter();
    const users = new UsersService(audit);
    const user = users.create({ email: "compliance@example.com", displayName: "Compliance", roles: ["compliance_admin"] }, superAdminActor);
