...
describe("broker Starter lead detail", () => {
  it("marks first authorized detail read as seen and refuses cross-tenant reads", async () => {
    const audit = new AuditLogWriter();
    const leads = new LeadsModule(new PartnersService(audit), new PartnerLicensesService(audit), audit);
    const assignment = await leads.assignments.create({
      quoteRequestId: "q1",
    ...
    const first = await leads.brokerStarterController.detail(assignment.id, actor);
    const second = await leads.brokerStarterController.detail(assignment.id, actor);

    expect(first.seen).toBe(true);
    expect(second.history.filter((event) => event.eventType === "viewed")).toHaveLength(1);
    await expect(leads.brokerStarterController.detail(assignment.id, { ...actor, partnerTenantId: "broker-b" })).rejects.toThrow("Lead access denied");
