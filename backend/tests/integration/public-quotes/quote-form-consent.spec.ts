...
describe("quote form and consent", () => {
  it("exposes published form with consent reference and records consent before routing", async () => {
    ...
    const form = seed.app.quoteForms.publicController.get(seed.countryId, seed.productId);
    ...
    expect(seed.app.consent.service.searchRecords(superAdminActor)).toHaveLength(1);
